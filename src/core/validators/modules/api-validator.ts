...
export class APIValidator extends BaseValidator {
   ...
   */
  static validateUsernameRequest(
    request: any,
      ...
      for (let i = 0; i < request.items.length; i++) {
        ...
        if (item.username) {
          ...
          if (!usernameResult.isValid) {
            errors.push(
              ...usernameResult.errors.map((error) => `Item ${i}: ${error}`),
            );
          }
        } else if (item.text) {
          ...
          if (!textResult.isValid) {
            errors.push(
              ...textResult.errors.map((error) => `Item ${i}: ${error}`),
            );
          }
        } else {
  ...
  private static validateCommonOptions(options: any): string[] {
    ...
    if (
      options.includeMetrics !== undefined &&
      typeof options.includeMetrics !== "boolean"
    ) {
