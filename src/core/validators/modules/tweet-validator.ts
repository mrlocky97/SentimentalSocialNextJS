...
export class TweetValidator extends BaseValidator {
   ...
   */
  private static validateContent(tweet: any, options: ValidationOptions): string[] {
    const errors: string[] = [];
  ...
  private static validateAuthor(author: any): string[] {
    ...
    if (author.followersCount !== undefined) {
      errors.push(
        ...this.validateNumberRange(author.followersCount, 'author.followersCount', {
          min: 0,
          max: 1000000000,
        })
      );
  ...
  static validateBatch(tweets: any[], options: ValidationOptions = {}): ValidationResult {
    ...
    for (let i = 0; i < tweets.length; i++) {
      ...
      warnings.push(...result.warnings.map((warning) => `Tweet ${i}: ${warning}`));
    }
