  ...
  static map(
    ...
    const apiResponse: TweetAnalysisAPIResponse = {
      ...
      sentiment: {
        ...
        confidence:
          Math.round(analysis.analysis.sentiment.confidence * 1000) / 1000,
        emotions: analysis.analysis.sentiment.emotions
          ? {
              joy:
                Math.round(analysis.analysis.sentiment.emotions.joy * 1000) /
                1000,
              sadness:
                Math.round(
                  analysis.analysis.sentiment.emotions.sadness * 1000,
                ) / 1000,
              anger:
                Math.round(analysis.analysis.sentiment.emotions.anger * 1000) /
                1000,
              fear:
                Math.round(analysis.analysis.sentiment.emotions.fear * 1000) /
                1000,
              surprise:
                Math.round(
                  analysis.analysis.sentiment.emotions.surprise * 1000,
                ) / 1000,
              disgust:
                Math.round(
                  analysis.analysis.sentiment.emotions.disgust * 1000,
                ) / 1000,
            }
          : undefined,
      },
      insights: {
        ...
        engagementPotential:
          Math.round(analysis.marketingInsights.engagementPotential * 1000) /
          1000,
        brandRisk: analysis.marketingInsights.brandRisk,
        marketingOpportunities:
          analysis.marketingInsights.viralityIndicators.slice(0, 5),
      },
      metadata: {
...
export class BatchAnalysisAPIMapper extends BaseAPIMapper {
  ...
  private static calculateBatchSummary(
    ...
    const sentimentDistribution = analyses.reduce(
      (dist, analysis) => {
        const label = this.normalizeSentimentLabel(
    ...
    const keywordFreq = new Map<
      string,
      { frequency: number; totalSentiment: number }
    >();
    analyses.forEach((analysis) => {
      analysis.analysis.keywords.forEach((keyword) => {
        const current = keywordFreq.get(keyword) || {
          frequency: 0,
          totalSentiment: 0,
        };
        keywordFreq.set(keyword, {
          ...
          totalSentiment:
            current.totalSentiment + analysis.analysis.sentiment.score,
        });
      });
    });
    ...
    const topKeywords = Array.from(keywordFreq.entries())
      .map(([keyword, data]) => ({
        ...
        avgSentiment:
          Math.round((data.totalSentiment / data.frequency) * 1000) / 1000,
      }))
...
export class ComparisonAPIMapper extends BaseAPIMapper {
  ...
  private static generateRecommendation(comparison: any): string {
    ...
    const confidences = methods.map(
      (method) => comparison[method]?.confidence || 0,
    );
