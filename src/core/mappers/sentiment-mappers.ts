...
export abstract class BaseSentimentMapper {
   ...
   */
  protected static normalizeSentimentLabel(
    label: string,
    ...
    if (lowerLabel.includes("positive") || lowerLabel === "very_positive") {
      ...
      lowerLabel.includes("negative") ||
      lowerLabel === "very_negative"
    ) {
      return "negative";
    } else {
      return "neutral";
  ...
  protected static normalizeScore(
    ...
    if (originalRange) {
      ...
      const normalized =
        ((score - originalRange.min) /
          (originalRange.max - originalRange.min)) *
          2 -
        1;
...
export class TweetSentimentAnalysisMapper extends BaseSentimentMapper {
  ...
  static mapBatch(
    ...
    return tweets.map((tweet, index) =>
      this.map(tweet, analyses[index], options),
    );
  }
   ...
   */
  private static generateMarketingInsights(
    tweet: Tweet,
    analysis: AnalysisResult,
  ) {
    const text = (tweet.content || tweet.text || "").toLowerCase();
   ...
   */
  private static calculateEngagementPotential(
    tweet: Tweet,
    analysis: AnalysisResult,
  ): number {
   ...
   */
  private static detectViralityIndicators(
    tweet: Tweet,
    analysis: AnalysisResult,
  ): string[] {
    const indicators = [];
  ...
  private static calculateTrendAlignment(tweet: Tweet): number {
    ...
    const hasPopularHashtags = hashtags.some((tag) =>
      ["trending", "viral", "popular"].some((keyword) =>
   ...
   */
  private static evaluateBrandRisk(
    analysis: AnalysisResult,
  ): "low" | "medium" | "high" {
  ...
  private static calculateOpportunityScore(
    ...
    return (
      engagementPotential * 0.4 + trendAlignment * 0.3 + sentimentFactor * 0.3
    );
  }
...
export class SentimentStatsMapper extends BaseSentimentMapper {
  ...
  static mapFromAnalyses(analyses: TweetSentimentAnalysis[]) {
    ...
    const averageSentiment =
      sentimentScores.reduce((sum, score) => sum + score, 0) / totalAnalyzed;
    ...
    analyses.forEach((analysis) => {
      analysis.brandMentions.forEach((mention) => {
        const current = brandStats.get(mention.brand) || {
          mentions: 0,
          totalSentiment: 0,
        };
