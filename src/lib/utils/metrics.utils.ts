...
export function calculateEvaluationMetrics(
    ...
    const macroPrecision = labels.reduce((sum, label) => sum + metrics[label].precision, 0) / labels.length;
    const macroRecall = labels.reduce((sum, label) => sum + metrics[label].recall, 0) / labels.length;
    const macroF1Score = labels.reduce((sum, label) => sum + metrics[label].f1Score, 0) / labels.length;

    // Calculate accuracy
    const correctPredictions = actual.filter((actualLabel, index) => actualLabel === predicted[index]).length;
...
export function calculateCampaignPerformance(metrics: {
    ...
    // Volume score (30%)
    const volumeScore = Math.min(100, (metrics.totalTweets / Math.max(1, metrics.targetReached)) * 100);
    ...
    const timeScore = metrics.timeRemaining > 0 ? 100 : Math.max(0, 100 - Math.abs(metrics.timeRemaining) * 10);
