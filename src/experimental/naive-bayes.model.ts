...
export class NaiveBayesSentimentModel {
  ...
  async train(trainingData: TrainingData[]): Promise<void> {
    console.log('🧠 Iniciando entrenamiento Naive Bayes...');
    console.log(`📊 Dataset: ${trainingData.length} documentos`);
  ...
  private async buildVocabulary(trainingData: TrainingData[]): Promise<void> {
    ...
    console.log(`📚 Vocabulario construido: ${this.statistics.vocabulary.size} palabras únicas`);
    console.log(`🔥 Palabras filtradas por frecuencia mínima (${this.config.minWordFrequency}): ${Array.from(wordFrequency.entries()).length - sortedWords.length}`);
  }
  ...
  private async calculateProbabilities(trainingData: TrainingData[]): Promise<void> {
    ...
    console.log(`🔢 Probabilidades calculadas para ${this.statistics.wordProbabilities.size} palabras`);
  }
  ...
  private printTrainingStatistics(): void {
    ...
    console.log('================================');
    console.log(`📚 Total documentos: ${this.statistics.totalDocuments}`);
    console.log(`✅ Positivos: ${this.statistics.documentCounts.positive} (${(this.statistics.classPriors.positive * 100).toFixed(1)}%)`);
    console.log(`❌ Negativos: ${this.statistics.documentCounts.negative} (${(this.statistics.classPriors.negative * 100).toFixed(1)}%)`);
    console.log(`⚪ Neutrales: ${this.statistics.documentCounts.neutral} (${(this.statistics.classPriors.neutral * 100).toFixed(1)}%)`);
    console.log(`🔤 Vocabulario: ${this.statistics.vocabulary.size} palabras únicas`);
    console.log(`⚙️  Suavizado: ${this.config.smoothingFactor}`);
  }
  ...
  importModel(modelData: any): void {
    ...
    console.log(`✅ Modelo importado exitosamente (exportado: ${modelData.exportDate})`);
  }
}
