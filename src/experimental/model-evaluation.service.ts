    ...
    const simulatedModels: ModelComparison[] = [
      {
        ...
        metrics: {
          ...
          cohen_kappa: 0.837,
          confusion_matrix: [[7, 1, 0], [1, 6, 1], [0, 1, 7]],
      ...
      },
      {
        ...
        metrics: {
          ...
          cohen_kappa: 0.739,
          confusion_matrix: [[6, 1, 1], [1, 6, 1], [1, 1, 6]],
          classification_report: 'Detailed classification report...',
      ...
      },
      {
        ...
        metrics: {
          ...
          cohen_kappa: 0.870,
          confusion_matrix: [[7, 1, 0], [0, 7, 1], [0, 1, 7]],
          classification_report: 'Detailed classification report...',
  ...
  async runCompleteExperiment(): Promise<ExperimentalResults> {
    ...
    return {
      ...
      dataset_info: {
        ...
        positive_samples: this.groundTruthData.filter(d => d.true_sentiment === 'positive').length,
        negative_samples: this.groundTruthData.filter(d => d.true_sentiment === 'negative').length,
        neutral_samples: this.groundTruthData.filter(d => d.true_sentiment === 'neutral').length,
        source: 'manually_annotated_mixed_domain',
        quality_score: this.groundTruthData.reduce((sum, d) => sum + d.annotator_agreement, 0) / this.groundTruthData.length
      },
      ...
      recommendations: [
        ...
        isSignificant ? 'Improvement is statistically significant (p < 0.05)' : 'Improvement is not statistically significant',
