...
userSchema.pre('save', function(this: IUserDocument, next) {
  ...
  if (this.isNew && this.permissions.length === 0) {
    switch (this.role) {
      case 'admin':
        this.permissions = ['campaigns:create', 'campaigns:edit', 'campaigns:delete', 'campaigns:view', 'users:manage', 'analytics:view'];
        break;
      case 'manager':
        this.permissions = ['campaigns:create', 'campaigns:edit', 'campaigns:view', 'analytics:view'];
        break;
      case 'analyst':
