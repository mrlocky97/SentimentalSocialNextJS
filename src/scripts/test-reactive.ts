  ...
  try {
    ...
    try {
      const reactiveResults = await firstValueFrom(
        reactiveSentimentAnalyzer
          .analyzeTweetsBatch(testTweets)
          .pipe(timeout(500)),
      );
      console.log(
        `   ✅ Reactive Sentiment: ${reactiveResults.length} tweets processed`,
      );
    } catch (error) {
    ...
    try {
      const scrapeResults = await firstValueFrom(
        reactiveTwitterScraper
          .batchScrape(["#test"], {}, "low")
          .pipe(timeout(500)),
      );
      console.log(
        `   ✅ Reactive Scraper: ${scrapeResults.length} items processed`,
      );
    } catch (error) {
    ...
    const executionTime = (endTime - startTime) / 1000;
    console.error(
      `❌ Reactive test failed after ${executionTime.toFixed(2)} seconds:`,
      error,
    );
    process.exit(1);
  }
