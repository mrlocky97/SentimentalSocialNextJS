...
export class MultiLanguageSentimentAnalyzer {
  ...
  private initializeLexicons() {
    ...
    this.languageLexicons.set('es', {
      ...
      negations: ['no', 'nunca', 'jamás', 'nada', 'nadie', 'ningún', 'ninguna', 'sin'],
    });
    ...
    this.languageLexicons.set('fr', {
      ...
      negations: ['ne', 'pas', 'non', 'jamais', 'rien', 'personne', 'aucun', 'aucune'],
    });
    ...
    this.languageLexicons.set('de', {
      ...
      intensifiers: ['sehr', 'extrem', 'unglaublich', 'absolut', 'völlig', 'total'],
      negations: ['nicht', 'nein', 'nie', 'niemals', 'nichts', 'niemand', 'kein', 'keine'],
    });
    ...
    this.languageLexicons.set('pt', {
      ...
      negations: ['não', 'nunca', 'jamais', 'nada', 'ninguém', 'nenhum', 'nenhuma', 'sem'],
    });
  }
  ...
  detectLanguage(text: string): string {
    ...
    if (/\b(el|la|los|las|un|una|de|en|con|por|para|que|se|no|es|está)\b/.test(lowerText)) {
      scores['es'] += 3;
    }
    if (/\b(le|la|les|un|une|de|du|des|et|ou|avec|pour|que|ne|est|sont)\b/.test(lowerText)) {
      scores['fr'] += 3;
    }
    if (/\b(der|die|das|den|dem|ein|eine|und|oder|mit|für|dass|ist|sind)\b/.test(lowerText)) {
      scores['de'] += 3;
    }
    if (/\b(il|la|lo|gli|le|un|una|di|in|con|per|che|non|è|sono)\b/.test(lowerText)) {
      scores['it'] += 3;
    }
  ...
  analyzeMultiLanguage(
    ...
    const lexicon = this.languageLexicons.get(detectedLang) || this.languageLexicons.get('en')!;
    ...
    const languageConfidence = Math.min(0.95, (languageMatches / words.length) * 2);
