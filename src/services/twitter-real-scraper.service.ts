...
export class TwitterRealScraperService {
  ...
  /**
   * Initialize and authenticate the scraper (ONLY ONCE)
   */
  private async initializeScraper() {
    ...
    try {
      ...
      console.log(`⏰ Login attempts: ${this.loginAttempts}/${this.maxLoginAttempts}`);
      
      if (this.loginAttempts >= this.maxLoginAttempts) {
        console.log(`🚫 Maximum login attempts reached. Waiting ${this.loginCooldown / 60000} minutes...`);
      }
