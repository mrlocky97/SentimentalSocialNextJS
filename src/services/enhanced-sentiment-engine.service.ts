...
export class EnhancedSentimentEngine {
  ...
  async analyzeEnhanced(
    ...
    const [ruleResult, naiveResult, vaderResult, textBlobResult] =
      await Promise.all([
    ...
    const modelPredictions: ModelPrediction[] = [
      {
        label: ruleResult.sentiment.label as
          | "positive"
          | "negative"
          | "neutral",
      ...
      },
      {
        ...
        score:
          ...
          (naiveResult.label === "positive"
            ? 0.7
            : naiveResult.label === "negative"
              ? -0.7
              : 0),
        method: "Naive Bayes",
      },
    ...
    const features = this.hybridAnalyzer.extractContextualFeatures(
      text,
      language,
    );
  ...
  private calculateConsensus(
    ...
    const agreementCount = predictions.filter(
      (p) => p.label === finalPrediction.label,
    ).length;
  ...
  async loadAndTrain(): Promise<void> {
    try {
      ...
      console.log(
        `📊 Using ${enhancedTrainingDataV3.length} enhanced training examples`,
      );
