...
export class DatasetExpansionService {
  ...
  async downloadDataset(source: DatasetSource): Promise<string> {
    ...
    try {
      ...
      console.log(`📊 Tamaño: ${(response.data.length / 1024 / 1024).toFixed(2)} MB`);
  ...
  async processDataset(source: DatasetSource, filepath: string): Promise<ProcessedDataset> {
    ...
    try {
      ...
      console.log(`✅ Procesado: ${stats.total} ejemplos`);
      console.log(`📊 Distribución: ${stats.positive}P / ${stats.negative}N / ${stats.neutral}Neu`);
  ...
  async processMultipleDatasets(sourceNames?: string[]): Promise<ProcessedDataset[]> {
    ...
    for (const source of targetSources) {
      try {
        ...
        console.log(`🌐 Idiomas: ${source.languages.join(', ')}`);
        ...
        console.log(`✅ Completado: ${source.name}\n`);
  ...
  combineDatasets(datasets: ProcessedDataset[], maxPerSource: number = 1000): TrainingData[] {
    ...
    for (const dataset of datasets) {
      console.log(`📦 ${dataset.source}: ${dataset.stats.total} ejemplos`);
      ...
      console.log(`   ✅ Añadidos: ${limited.length} ejemplos`);
    }

    // Mezclar aleatoriamente
    const shuffled = this.shuffleArray(combined);
    
    console.log(`\n🎯 Total combinado: ${shuffled.length} ejemplos`);
...
export function getExpandedDatasetStats() {
  return EXPANDED_DATASET_STATS;
    ...
    console.log(`💾 Dataset guardado: ${filepath}`);
...
export async function expandDataset(sourceNames?: string[], maxPerSource: number = 1000) {
  console.log('🚀 INICIANDO EXPANSIÓN DEL DATASET');
  console.log('==================================\n');
  ...
  try {
    ...
    console.log('\n🎉 ¡Expansión del dataset completada!');
    console.log(`📊 Total: ${combined.length} ejemplos`);
