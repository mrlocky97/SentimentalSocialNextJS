...
export class SentimentAnalysisService implements SentimentAnalysisProvider {
  ...
  private async analyzeSentiment(text: string): Promise<SentimentResult> {
    ...
    let magnitude = 0;
    let wordCount = 0;
    const sentimentWords: Array<{ word: string; score: number; position: number }> = [];
    ...
    for (const sentimentWord of sentimentWords) {
      ...
      for (let j = Math.max(0, pos - 3); j < pos; j++) {
        ...
        if (intensifiers.includes(prevWord)) {
          ...
          if (
            ['extremely', 'incredibly', 'absolutely', 'tremendously', 'enormously'].includes(
              prevWord
            ) ||
            ['extremadamente', 'increíblemente', 'absolutamente', 'tremendamente'].includes(
              prevWord
            )
          ) {
            intensity = 2.0;
          } else if (
            ['very', 'really', 'super', 'quite', 'pretty'].includes(prevWord) ||
            ['muy', 'súper', 'bastante', 'realmente'].includes(prevWord)
          ) {
            intensity = 1.5;
          } else if (
            ['somewhat', 'rather', 'fairly', 'slightly'].includes(prevWord) ||
    ...
    const confidence = Math.min(
      0.95,
      Math.max(0.1, density * 0.4 + textLengthFactor * 0.3 + magnitudeFactor * 0.3)
    );
  ...
  private analyzeEmotions(text: string, sentimentScore: number): EmotionAnalysis {
    ...
    for (const [emotion, keywords] of Object.entries(emotionKeywords)) {
      ...
      keywords.forEach((keyword) => {
        ...
        const partialMatches =
          (lowerText.match(new RegExp(keyword, 'g')) || []).length - exactMatches;
    ...
    Object.keys(emotions).forEach((key) => {
      emotions[key as keyof EmotionAnalysis] = Math.min(
        1,
        Math.max(0, emotions[key as keyof EmotionAnalysis])
      );
    });
  ...
  private preprocessText(text: string): string {
    ...
    const contractions: { [key: string]: string } = {
      ...
      "l'est": 'le est',
      "d'accord": 'de accord',
      "d'habitude": 'de habitude',
      "d'ailleurs": 'de ailleurs',
      "qu'est": 'que est',
      "qu'il": 'que il',
      "qu'elle": 'que elle',
      "n'est": 'ne est',
