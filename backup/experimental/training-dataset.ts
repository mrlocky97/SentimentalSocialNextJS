...
export function getDatasetStatistics() {
  const stats = {
    ...
    languages: {
      spanish: SENTIMENT_TRAINING_DATASET.filter(
        (d) =>
          /[áéíóúüñ]/.test(d.text) ||
          /\b(el|la|es|de|en|con|por|para|que|un|una|muy|más|pero|como|todo|bien|mejor|bueno|malo|producto|servicio)\b/i.test(
            d.text
          )
      ).length,
      english: SENTIMENT_TRAINING_DATASET.filter(
        (d) =>
          !/[áéíóúüñ]/.test(d.text) &&
          /\b(the|a|an|is|of|in|with|for|that|and|or|but|this|product|service|quality|good|bad|great|excellent)\b/i.test(
            d.text
          )
      ).length,
    },
 ...
 */
export function splitDataset(testRatio: number = 0.2): {
  train: TrainingData[];
  test: TrainingData[];
} {
