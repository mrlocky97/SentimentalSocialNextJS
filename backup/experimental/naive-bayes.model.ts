...
export class NaiveBayesSentimentModel {
  ...
  private async calculateProbabilities(trainingData: TrainingData[]): Promise<void> {
    ...
    for (const [word, counts] of wordClassCounts.entries()) {
      ...
      this.statistics.wordProbabilities.set(word, {
        positive:
          (counts.positive + smoothing) /
          (totalPositive + smoothing * this.statistics.vocabulary.size),
        negative:
          (counts.negative + smoothing) /
          (totalNegative + smoothing * this.statistics.vocabulary.size),
        neutral:
          (counts.neutral + smoothing) /
          (totalNeutral + smoothing * this.statistics.vocabulary.size),
        count: counts.count,
      });
    }
  }
   ...
   */
  private calculateLogProbabilities(tokens: string[]): {
    positive: number;
    negative: number;
    neutral: number;
  } {
   ...
   */
  private selectBestClass(logProbs: {
    positive: number;
    negative: number;
    neutral: number;
  }): SentimentLabel {
   ...
   */
  private convertLogProbabilities(logProbs: {
    positive: number;
    negative: number;
    neutral: number;
   ...
   */
  private calculateConfidence(probs: {
    positive: number;
    negative: number;
    neutral: number;
  }): number {
    ...
    const secondMaxProb = Math.max(
      maxProb === probs.positive
        ? Math.max(probs.negative, probs.neutral)
        : Math.max(probs.positive, probs.negative, probs.neutral),
      0
    );
   ...
   */
  private calculateSentimentScore(probs: {
    positive: number;
    negative: number;
    neutral: number;
  }): number {
  ...
  private printTrainingStatistics(): void {
    console.log(
      `❌ Negativos: ${this.statistics.documentCounts.negative} (${(this.statistics.classPriors.negative * 100).toFixed(1)}%)`
    );
  }
