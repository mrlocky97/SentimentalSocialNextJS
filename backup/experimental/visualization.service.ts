...
export class VisualizationService {
  ...
  generateVisualizationReport(results: ExperimentalResults): VisualizationReport {
    const charts: ChartData[] = [
      ...
      this.generateConfusionMatrixHeatmap(
        results.models_compared.find((m) => m.model_name === results.best_model.name)!
      ), // Best model
    ...
    const bestModel = results.models_compared.find(
      (m) => m.model_name === results.best_model.name
    )!;
    const performanceImprovement = (
      (bestModel.metrics.accuracy - baselineModel.metrics.accuracy) *
      100
    ).toFixed(1);
  ...
  private analyzeSpeedAccuracyTradeoff(models: ModelComparison[]): string {
    ...
    const speedDiff =
      mostAccurateModel.metrics.processing_time_ms / fastestModel.metrics.processing_time_ms;
  ...
  private analyzeModelComplexity(models: ModelComparison[]): string {
    ...
    const complexityAnalysis = models
      .map((m) => ({
        name: m.model_name,
        type: m.model_type,
